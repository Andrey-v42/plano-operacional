...
import { Card, Input, Button, List, Avatar, Badge, Typography, Space, Divider, Empty, Spin, Select, notification } from 'antd';
import { SendOutlined, UserOutlined, CommentOutlined, CustomerServiceOutlined, InfoCircleOutlined, SearchOutlined, ExclamationCircleOutlined } from '@ant-design/icons';
import { Row, Col } from 'antd';

const { Text, Title } = Typography;
const { TextArea } = Input;

const ChatTab = ({ pipeId, ticketId, autoCreate, onChatCreated }) => {
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            await fetch('https://southamerica-east1-zops-mobile.cloudfunctions.net/addDocChat', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const createNewChat = async () => {
        if (selectedTicket === null && ticketId === null) {
            openNotificationFailure('Selecione um ticket para criar um novo chat.');
        ...
        try {
            ...
            let equipeSuporte = dataEvento.equipeEscalada.map((item) => {
                if ((item.funcao === 'C-CCO' || item.funcao === 'Head') && item.nome !== localStorage.getItem('currentUser')) {
                    return item.nome
                }
            });

            equipeSuporte = equipeSuporte.filter(item => item !== undefined)

            const currentUser = localStorage.getItem('currentUser');
            const formattedUsername = currentUser.split(' ')[0]
            const newChat = {
                title: `${formattedUsername} - ${new Date().toLocaleString()}`,
                participants: [currentUser, ...equipeSuporte],
                created: Date.now(),
                createdBy: currentUser,
                ticketId: selectedTicket,
                lastTimestamp: Date.now(),
                lastMessage: 'Chat iniciado na data: ' + new Date().toLocaleString(),
                lastSender: currentUser,
                unread: 0
            };

            const response = await fetch('https://southamerica-east1-zops-mobile.cloudfunctions.net/addDocChat', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        const fetchTickets = async () => {
            try {
                ...
                data = data.docs.map((doc) => {
                    const data = doc.data;
                    if (data.status !== 'closed' && data.solicitante == localStorage.getItem('currentUser')) {
                        return {
                            value: doc.id,
                            label: `${data.categoria} - ${doc.id}`,
                        }
                    }
                })
                data = data.filter(item => item !== undefined)
                console.log(data)
                setTicketsOptions(data);
            } catch (error) {
                console.error(error);
            }
        }
    ...
    return (
        <>
            {contextHolder}
            <div className="chat-container" style={{ height: '90vh', width: '80vw', display: 'flex', flexDirection: 'column' }}>
                {/* Navigation Header */}
                <div className="chat-header" style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <Select options={ticketsOptions} style={{ minWidth: '200px', marginRight: '1vh', marginLeft: 'auto' }} placeholder="Selecione o Ticket" onChange={(value) => setSelectedTicket(value)} />
                        <Button
                            type="primary"
                        ...
                        <div style={{
                            ...
                            {!selectedChat ? (
                                <div style={{
                                    ...
                                    <Select options={ticketsOptions} style={{ marginBottom: '2vh', minWidth: '175px' }} placeholder="Selecione o Ticket" onChange={(value) => setSelectedTicket(value)} />
                                    <Button
                                        type="primary"
                                        onClick={createNewChat}
                                        size="large"
